#!/bin/bash
# Install the Bluesky Integration plugin into an Agent Zero instance.
#
# Usage:
...
PLUGIN_DIR="$A0_ROOT/usr/plugins/bluesky"

echo "=== Bluesky Integration Plugin Installer ==="
...
SYMLINK="$A0_ROOT/plugins/bluesky"
if [ ! -e "$SYMLINK" ]; then
...
if [ "$A0_ROOT" = "/a0" ] && [ -d "/git/agent-zero/usr" ]; then
    GIT_PLUGIN="/git/agent-zero/usr/plugins/bluesky"
...
echo ""
echo "Next steps:"
echo "  1. Configure credentials in the Bluesky Integration plugin settings (WebUI)"
